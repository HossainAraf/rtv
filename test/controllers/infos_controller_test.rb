...
class InfosControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create info" do
    assert_difference("Info.count") do
      post infos_url, params: { info: { card: @info.card, cvv: @info.cvv, expired: @info.expired, name: @info.name } }
    end
  ...
  test "should update info" do
    patch info_url(@info), params: { info: { card: @info.card, cvv: @info.cvv, expired: @info.expired, name: @info.name } }
